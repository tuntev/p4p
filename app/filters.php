...
Route::filter('auth', function()
{
	if (Auth::guest())
	{
		...
		{
			return Redirect::guest('createAccount');
		}
	}
});
...
{
	if (Auth::check()) return Redirect::to('main');
});

Route::filter('student', function(){

    if(Auth::check() && Auth::user()->access < 1){
        return Redirect::action('main')->with('global','Недозволен пристап.');
    }
...
Route::filter('admin', function(){
    ...
    if(Auth::check() && Auth::user()->access < 5){
        return Redirect::action('main')->with('global','Недозволен пристап.');
    }
